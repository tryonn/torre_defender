...
var _efeitoTiro: GameObject;
var _p: Transform;

function Start ()
{
	_alcance = 30;//alcance do tira e de 30 m
	_poder_ataque = 6;//tira 10 de vida do inimigo
	_tempo_ataque = 0.7;//ataca de 1 em 1 segundo
}
...
function Update ()
{
	...
	if(_achouInimigo)
	{
		_mira = transform.GetChild(0);//filho zero da mira
		_mira = _mira.GetChild(0);//filho zero da mira recebe o filho zero dele
		
		_p = _mira.GetChild(0);// filho zero da mira
		_p = _p.GetChild(0);//filho zero da mira recebe o filho zero dele
		
		
		var _rotacao: Quaternion;
		_rotacao = Quaternion.LookRotation(transform.position - _auxInimigo.transform.position);
		_mira.rotation.eulerAngles.y = _rotacao.eulerAngles.y - 90;
		
		if(_tempo > _tempo_ataque)
		{
			Instantiate(_efeitoTiro, _p.position, _p.rotation);
